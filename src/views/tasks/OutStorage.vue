  ...
  <div class="putbig">
    ...
    <div class="shebei-content">
      ...
      <el-table
        ...
        <el-table-column
          prop="outstorage_person"
          label="出库人"
          show-overflow-tooltip
          width="120">
        </el-table-column>
        <el-table-column
          prop="outstorage_time"
          label="出库时间"
          show-overflow-tooltip
          width="120">
        </el-table-column>
        ...
        <el-table-column
          ...
          width="80">
            <template slot-scope="scope">
              <el-button type="warning" :disabled="disable" icon="el-icon-edit-outline" size="mini" plain @click="outopen(scope.row.id)"></el-button>
            </template>
        </el-table-column>
      </el-table>
      <el-button plain class="rukudan-btn" type="primary" size="small" @click="handlerukudan">打印出库单</el-button>
...
export default {
  ...
  methods: {
    // 打开设备dialog
    outopen(id) {
      this.$store.dispatch("outstoragedialog", id);
    },
    // 任务进度改变
    handletasksechedule() {
    ...
    handleputstorage() {
      this.$confirm("确定提交任务吗", "提示", {
        ...
        .then(async () => {
          const res = await this.$http.put(`outstorage/${this.taskid}`);
